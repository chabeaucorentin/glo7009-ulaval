...
if ($image) {
    ...
    if ($image["error"] == UPLOAD_ERR_NO_FILE) {
        $error = "Veuillez mettre en ligne une image !";
    } else if (!$image["error"] && !move_uploaded_file($image["tmp_name"], "uploads/".$image_name)) {
    ...
    <form enctype="multipart/form-data" method="POST">
        <div>
            ...
            <div class="form-group">
                <label for="profile">Document :</label>
                '.((isset($error)) ? '<div class="alert">'.$error.'</div>' : '').'
                <input id="profile" class="dropify'.((isset($error)) ? ' invalid' : '').'" name="profile" type="file" />
            </div>
        </div>
        <footer>
